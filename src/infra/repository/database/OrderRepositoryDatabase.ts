...
export default class OrderRepositoryDatabase implements OrderRepository {
	...
	async save(order: Order): Promise<void> {
		const [orderData] = await this.connection.query("insert into ccca.order (code, cpf, issue_date, total, freight) values ($1, $2, $3, $4, $5) returning *", [order.getCodeOrder(), order.cpf.getValue(), order.date, order.getTotal(), order.freight]);
